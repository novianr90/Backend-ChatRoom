...
public class Group {
    ...
    // }
    public void SendMessageToMembers(SimpMessagingTemplate simpMessagingTemplate, Message message) {
    System.out.println("DEBUG: Memulai broadcast untuk grup: " + this.id);
    
    for (User user : members) {
        System.out.println("DEBUG: Memproses member: " + user.getUsername());
        
        // KUNCI UTAMANYA DI SINI: Gunakan nama user, BUKAN this.id
        String targetJalur = "/topic/user/" + user.getUsername(); 
        
        System.out.println("DEBUG: Menembakkan pesan ke: " + targetJalur);
        
        // Kirim pesan ke targetJalur tersebut
        simpMessagingTemplate.convertAndSend(targetJalur, message);
    }
}
